...
def main():
    ...
    for test_domain in DOMAINS:
        ...
        for domain_name in train_domains:  # Only iterate over actual domains
            domain_results = phase1_results[domain_name]
            # Visualize final weights and OBD saliencies
            visualize_phase1_domain_obd(
    ...
    for test_domain, train_results in all_phase1_results.items():
        for train_domain in [d for d in DOMAINS if d != test_domain]:
            if train_domain in train_results:
                flattened_phase1[f"{train_domain} (for {test_domain} holdout)"] = train_results[train_domain]
