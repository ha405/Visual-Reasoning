...
def visualize_phase1_domain(domain_results, domain_name, save_dir=None):
    """
    Create all Phase 1 visualizations for a single domain:
    1. Initial weights
    2. Final weights
    3. Continuous importance (0-1)
    4. Binary masks (0 or 1)
    ...
    visualize_weight_matrices_grid(
        ...
        save_path=f'{save_dir}/{domain_name}_continuous_importance.png' if save_dir else None,
        value_range=(0, 1)
    )
...
def plot_training_curves(train_losses, train_accs, val_accs=None, title="Training Curves", save_path=None):
    """
...
def plot_lodo_results(results_dict, save_path=None):
    ...
    bars = ax.bar(domains, test_accs, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    ax.axhline(avg_acc, color='red', linestyle='--', linewidth=2, label=f'Average: {avg_acc:.4f}')
...
def plot_phase2_all_training_curves(all_results, save_dir=None):
    ...
    for test_domain, results in all_results.items():
        plot_training_curves(
            ...
            save_path=f'{save_dir}/phase2_training_{test_domain}.png' if save_dir else None
        )
...
def create_summary_report(phase1_results, phase2_results, save_path=None):
    ...
    for domain, results in phase1_results.items():
        ...
        report_lines.append(f"Domain: {domain:20s} | Final Loss: {final_loss:.4f} | Final Acc: {final_acc:.4f}")
    report_lines.append("")
    ...
    for test_domain, results in phase2_results.items():
        ...
        report_lines.append(f"Test Domain: {test_domain:20s} | Test Accuracy: {test_acc:.4f}")
    
    avg_test_acc = np.mean(test_accs)
    std_test_acc = np.std(test_accs)
    report_lines.append("")
    report_lines.append(f"Average Test Accuracy: {avg_test_acc:.4f} ± {std_test_acc:.4f}")
    report_lines.append("="*80)
