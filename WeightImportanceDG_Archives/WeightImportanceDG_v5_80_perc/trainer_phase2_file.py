...
class Phase2Trainer:
    ...
    def __init__(self, model, importance_masks, device=DEVICE):
        ...
        self.optimizer = optim.Adam(
            self.model.classifier.hidden_layers.parameters(), lr=PH2_LEARNING_RATE
        )
    ...
    def train(
        self, train_loader, val_loader, num_epochs=PH2_NUM_EPOCHS, test_domain="unknown"
    ):
        """
        ...
        print(f"{'='*60}")
        print(
            f"Training only the top {IMPORTANCE_THRESHOLD_PERCENTILE}% important weights"
        )
...
def run_phase2_lodo(dataset_handler, test_domain, importance_masks):
    ...
    train_loader, val_loader, test_loader = dataset_handler.get_lodo_loaders(
        test_domain=test_domain, batch_size=PH2_BATCH_SIZE
    )
