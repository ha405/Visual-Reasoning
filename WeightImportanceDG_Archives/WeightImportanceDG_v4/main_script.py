...
from importance_file import ImportanceCalculator, aggregate_importance_from_raw_changes, compute_mask_statistics
...
def main():
    ...
    for test_domain in DOMAINS:
        ...
        # ========================================================================
        # Aggregate Importance Masks (NEW APPROACH)
        # ========================================================================
        print("\n>>> Aggregating importance by summing weight changes across domains...")
        
        # Collect raw importance matrices (|final - initial|) from all training domains
        importance_matrices_dict = {
            domain: phase1_results[domain]['importance_matrices']
            for domain in train_domains
        }
        
        # Aggregate using element-wise sum across domains
        summed_importance, normalized_importance, aggregated_masks = aggregate_importance_from_raw_changes(
            importance_matrices_dict
        )
        
        # Visualize the aggregated (summed) importance
        print("\n>>> Generating aggregated importance visualizations...")
        aggregated_plot_dir = f"{PLOTS_DIR}/aggregated_{test_domain}_holdout"
        os.makedirs(aggregated_plot_dir, exist_ok=True)
        
        # Visualize summed importance (raw values)
        from visualization_file import visualize_weight_matrices_grid
        visualize_weight_matrices_grid(
            weight_matrices=summed_importance,
            title=f'Aggregated (Summed) Importance Across {train_domains}',
            save_path=f"{aggregated_plot_dir}/summed_importance.png"
        )
        
        # Visualize normalized importance
        visualize_weight_matrices_grid(
            weight_matrices=normalized_importance,
            title=f'Normalized Aggregated Importance (0-1)',
            save_path=f"{aggregated_plot_dir}/normalized_importance.png",
            value_range=(0, 1)
        )
        ...
        visualize_weight_matrices_grid(
            ...
            title=f'Binary Importance Masks (Top 60%) for Test Domain: {test_domain}',
            save_path=f"{aggregated_plot_dir}/binary_masks.png",
            value_range=(0, 1)
        )
        
        # Compute and print mask statistics
        mask_stats = compute_mask_statistics(aggregated_masks)
        print("\nAggregated Mask Statistics (from summed importance):")
        ...
        # ========================================================================
        print(f"\n>>> PHASE 2: Training pruned network (test on {test_domain})")
