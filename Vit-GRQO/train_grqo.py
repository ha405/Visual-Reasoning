...
from models import get_grqo_model
from dataset import get_dataset_class
from transform import get_transforms


def parse_args():
    parser = argparse.ArgumentParser(description="GRQO Training")
...
def run_lodo_grqo(args, cfg):
    ...
    for target_domain in domains:
        ...
        print(f"\n{'='*60}")
        print(f"LODO: Leaving out domain '{target_domain}'")
        print(f"{'='*60}")
        
        model = get_grqo_model(args.model, args.dataset, cfg).to(device)
        ...
        for epoch in range(1, cfg["train"]["epochs"] + 1):
            train_loss, train_cls, train_grqo, train_acc = train_epoch(
...
def train_epoch(model, train_loader, optimizer, device):
    from train import train_epoch as train_fn
    return train_fn(model, train_loader, optimizer, device)


def evaluate(model, val_loader, device):
    from train import evaluate as eval_fn
    return eval_fn(model, val_loader, device)
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            ...
            f"{args.model}_{args.dataset}_grqo"
        )
    
    os.makedirs(args.output_dir, exist_ok=True)
    os.makedirs(cfg["system"]["ckpt_dir"], exist_ok=True)
    
    with open(os.path.join(args.output_dir, "config.yaml"), "w") as f:
        yaml.dump(cfg, f)
    
    print(f"\n{'='*60}")
    print(f"GRQO Training")
    print(f"{'='*60}")
